...
class UserFollowerController: UITableViewController {
    var followers = [RelatedUserModel]()
    var user: SimpleUser?
    ...
    override func viewDidLoad() {
        ...
        if self.user != nil{
            self.navigationItem.title = "喜欢\(user!.name)的人"
        }else{
            self.navigationItem.title = "喜欢我的人"
        }
    ...
    func refreshStateChange(_ control:UIRefreshControl){
        ...
        IMApi.instance.readUserFollowers(uid!, pindex: page, psize: DEFAULT_PAGE_SIZE) { (resp) in
            if resp.isSuccess{
        ...
        IMApi.instance.readUserFollowers(uid!, pindex: page, psize: DEFAULT_PAGE_SIZE) { (resp) in
            if resp.isSuccess{
                if let marray = resp.Data{
                    ...
                    self.followers.append(contentsOf: marray)
                    self.tableView.reloadData()
                }
            }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        self.tableView.displayMsg(TABLEVIEW_NO_DATA_HINT, forRowCount: self.followers.count)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let follower = self.followers[(indexPath as NSIndexPath).row]
        ...
        if(!allDataLoaded){
            //加载更多
            if (indexPath as NSIndexPath).row == self.followers.count-1 {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let user = self.followers[(indexPath as NSIndexPath).row]
