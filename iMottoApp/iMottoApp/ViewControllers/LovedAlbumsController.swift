//
//  LoveAlbumsController.swift
//  iMottoApp
//
//  Created by sunht on 2016/11/28.
//  Copyright © 2016年 imotto. All rights reserved.
//

import UIKit

class LovedAlbumsController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.user != nil{
            self.navigationItem.title = "\(user!.name)喜欢的珍藏"
        }else{
            self.navigationItem.title = "我喜欢的珍藏"
        }
    ...
    func refreshStateChange(_ control:UIRefreshControl){
        ...
        IMApi.instance.readLovedAlbums(uid!, pindex: page, psize: DEFAULT_PAGE_SIZE){ (resp)
            in
            if resp.isSuccess{
