...
class UserBanController: UITableViewController {
    var bans = [RelatedUserModel]()
    ...
    func refreshStateChange(_ control:UIRefreshControl){
        ...
        let uid = PropHelper.instance.userId
        
        IMApi.instance.readUserBans(uid!, pindex: page, psize: DEFAULT_PAGE_SIZE) { (resp) in
            if resp.isSuccess{
        ...
        IMApi.instance.readUserBans(uid!, pindex: page, psize: DEFAULT_PAGE_SIZE) { (resp) in
            if resp.isSuccess{
                if let marray = resp.Data{
                    ...
                    self.bans.append(contentsOf: marray)
                    self.tableView.reloadData()
                }
            }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        self.tableView.displayMsg(TABLEVIEW_NO_DATA_HINT, forRowCount: self.bans.count)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let follower = self.bans[(indexPath as NSIndexPath).row]
        ...
        if(!allDataLoaded){
            //加载更多
            if (indexPath as NSIndexPath).row == self.bans.count-1 {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let user = self.bans[(indexPath as NSIndexPath).row]
